...
def transformLogEvent(log_event: dict[str, Union[str, int]], source: str) -> str:
...
def processRecords(records: list[dict[str, Union[str, int]]]):
    for r in records:
        ...
        if data["messageType"] == "CONTROL_MESSAGE":
            ...
            source = f"{data['logGroup']}:{data['logStream']}"
            data = "".join([transformLogEvent(e, source) for e in data["logEvents"]])
            encodedData = base64.b64encode(data.encode("utf-8")).decode()
...
def putRecordsToStream(isSas: bool, streamName: str, records: dict[str, Union[str, int]], client, attemptsMade: int, maxAttempts: int) -> None:
    failedRecords = []
    codes = []
    errMsg = ""
    service = "Kinesis" if isSas else "Firehose"
    ...
    if failedRecords and attemptsMade + 1 < maxAttempts:
        print(f"Some records failed while calling put records method to {service} stream, retrying. {errMsg}")
        putRecordsToStream(isSas, streamName, failedRecords, client, attemptsMade + 1, maxAttempts)
    else:
        raise RuntimeError(f"Could not put records after {maxAttempts} attempts. {errMsg}")


def createReingestionRecord(isSas: bool, originalRecord: list[dict[str, Union[str, int]]]):
    if isSas:
...
def getReingestionRecord(isSas: bool, reIngestionRecord: list[dict[str, Union[str, int]]]):
    if isSas:
...
def handler(event, context):
    ...
    if putRecordBatches:
        ...
        for recordBatch in putRecordBatches:
            putRecordsToStream(isSas, streamName, recordBatch, client, attemptsMade=0, maxAttempts=20)
            recordsReingestedSoFar += len(recordBatch)
            print(f"Reingested {recordsReingestedSoFar}/{totalRecordsToBeReingested} records out of {len(event['records'])}")
    else:
